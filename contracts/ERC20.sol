...
contract ERC20 is IERC20 {
    ...
    event log(string info, uint256 val);
    event log(string info, address addr, uint256 val);
    event log(string info, address addr1, address addr2, uint256 val);
    ...
    function init(string memory name_, string memory symbol_, uint256 total_) external override {
        name = name_;
        symbol = symbol_;
        total = total_;
    ...
    function totalSupply__() external {
        emit log("totalSupply", total);
    }

    function balanceOf(address account) external override view returns (uint256) {
        return balances[account];
    }
    function balanceOf__(address account) external {
        emit log("balance of account", account, balances[account]);
    }
    ...
    function logger__(string memory info) public {
        emit log(info);
    }
}